# Solenis-bot Frontend

The frontend of Solenis-bot AI Platform is built with Next.js, TypeScript, and Tailwind CSS, providing a modern and responsive user interface.
...
- **State Management**: Zustand (via `zustand` library for stores like `useMessagesStore`) and custom React hooks.
...
4. **Set up Environment Variables:**
   Create a `.env.local` file in the `final__solenis-botfrontend` root directory with the following variable. This URL should point to your running Solenis-bot backend.
   ```env
   NEXT_PUBLIC_API_BASE_URL=http://localhost:5000 # Corrected backend port
   ```
...
- **Efficient State Management**: Centralized state handling using Zustand for chat message state.
- **API Integration**: Seamless integration with the Solenis-bot backend via an Axios instance configured with `NEXT_PUBLIC_API_BASE_URL`.
...
- **`store/`**: Zustand store configurations, notably `messages.ts` for chat messages and state.

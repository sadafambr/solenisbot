    ...
    persist(
        (set, get) => ({
            ...
            saveMessage: async (message, response) => {
                ...
                try {
                    // Messages are persisted by backend /solenis-bot now.
                    return;
                } catch (error) {
            ...
            createNewChat: async (message) => {
                try {
                    const response = await api.post('/sessions/new', {
                        user_id: 1,
                        title: message.content?.slice(0, 80) || 'Untitled Chat'
                    });
            ...
            loadChatHistory: async (chatId: string) => {
                try {
                    const apiCallResponse = await api.get(`/sessions/${chatId}?user_id=1`);
